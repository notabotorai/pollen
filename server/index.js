...
// Geocode: city/zip → lat, lng using Nominatim (free, no key needed)
app.get('/api/geocode', async (req, res) => {
  const { q } = req.query;
  if (!q) return res.status(400).json({ error: 'Missing query parameter "q"' });

  try {
    // If the query looks like a US zip code, use the postalcode parameter for better results
...
function generateMockData(lat, lng, days) {
  ...
  // Use a seed based on lat/lng for consistent but varied results
...
app.listen(PORT, () => {
  console.log(`🌿 PollenWatch server running on http://localhost:${PORT}`);
  if (!GOOGLE_API_KEY) {
    console.warn('⚠️  No GOOGLE_POLLEN_API_KEY found — running in DEMO mode with mock data');
    console.warn('   Set GOOGLE_POLLEN_API_KEY in server/.env to use live data');
  }
});
